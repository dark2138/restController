    ...
    xhr.onreadystatechange = function(){
        if(xhr.readyState === 4 ){
            if(xhr.status === 200){
                ...
                let todos = JSON.parse(this.responseText); // responseText는 서버가 전송한 HTTP 요청에 대한 응답 문자열입니디
                // console.log(todos);
...
function deleteTodo(id){  // pathvaliable 아님 requestBody 임
    let delTodo = {"id": id};
    let xhr = new XMLHttpRequest();
...
function deleteTodo2(id){ // pathvaliable
//    let delTodo = {"id": id};
    let xhr = new XMLHttpRequest();
//    xhr.open('DELETE','http://localhost:8080/api/todos');
...
function todoItemAdd(todoObj){
    ...
    removeSpan.addEventListener('click',function(event){
        ...
        liObj.remove();
       // return false;
        // addEventListener 에서  불필요하며 실제로 아무런 효과가 없습니다.
        //  event.stopPropagation(); 로 전파 막으면 됨

        // return false를 하지 않으면 수정이 되면서 update를 호출한다.
        // 이미 삭제된 것을 수정하려고 하니 오류가 발생한다.
    });
};
